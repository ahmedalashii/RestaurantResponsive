...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Changa:wght@200;300;400;500;600;700;800&display=swap"
        rel="stylesheet">
    <!-- Css Files -->
    <link rel="stylesheet" href="../FP2/css/style.css">
...
<body>
                ...
                <div class="services-content">
                    ...
                    <div class="services-item">
                        ...
                        <p>المطعم هو مكان تقدم فيه الماكولات والمشروبات للزبائن. تم تشغيل المطاعم في بداية الأمر على
                            ...
                            والقطارات. وكذلك في المتنزهات والمباني التي تضم المكاتب وفي مراكز التسوق. تشكل المطاعم الجزء
                            الأكبر في صناعة خدعة الطعام وتشكل المحلات التي تقدم الطعام للناس خارج بيوتهم جزعا عن هذه
                            الصلاعة بما فيها المدارس والمستشفيات والمصانع والسجون.</p>
                    </div> <!-- ./services-item -->
                    <div class="services-item">
                ...
                <div class="get-category">
                    ...
                    <div>
                        <?php
                                ...
                                $sqlCountMeals = "SELECT count(id) FROM meals,categories WHERE meals.meal_category=categories.id GROUP BY meal_category";
                                $dataCountMeals = mysqli_query($conn, $sqlCountMeals);
                                $numOfMeals = mysqli_fetch_all($dataCountMeals);
                                if (mysqli_num_rows($dataCategories)>0) { // if there're categories then go ahead >>
                                    for ($i=0; $i < mysqli_num_rows($dataCountMeals); $i++) {
                                        $rowCategories=mysqli_fetch_assoc($dataCategories);
                                        if ($numOfMeals[$i][0]>0) { // if there're meals in each category >> print the category otherwise we don't want to print the category which doesn't have meals and that's very logical ^_^
                                            echo "<button class='category category-btn".$rowCategories['id']."' onclick='category".$rowCategories['id']."()'>$rowCategories[name]&nbsp;<span style='float:left;'>(".($numOfMeals[$i][0]).")</span></button>"; // $numOfMeals[$i][0] >> [0] is the column number one in the database table which is count(id) >> this is just to count number of meals in each category
                                        }
                                    }
                                }
                            ?>
    ...
    <section class="contact" id="contact">
        ...
        <div class="container">
            <div class="contact-info">
                ...
                <div class="subscribe">
                    <form action="" method="">
                        <label for="subs">اشترك معنا بالقائمة البريدية</label>
                        <input type="email" id="sub-email" placeholder="البريد الالكتروني">
                        <b id="subscribe-msg" style="display:inline-block;"></b>
                    </form>
                    <button class="btn" onclick="emailCheck()">اشترك الآن</button>
                </div> <!-- ./subscribe -->
            </div> <!-- ./contact-info -->
            ...
            <div class="send-message">
                <h3>أرسل رسالة الآن</h3>
                <form action="" method="post">
                    <input type="text" name="name" id="name" placeholder="الاسم">
                    <b id="name-msg"></b>
                    <input type="email" name="email" id="msg-email" placeholder="البريد الالكتروني">
                    <b id="email-msg"></b>
                    <textarea name="message" id="message" cols="30" rows="6" placeholder="الرسالة"></textarea>
                    <b id="msg"></b>
                </form>
                <button class="btn" onclick="messageCheck()">ارسل الآن</button>
            </div> <!-- ./send-message -->
        </div> <!-- ./container -->
    </section> <!-- ./contact -->
    ...
    <?php
                 $sqlCategories = "SELECT * FROM categories"; // 1,2,3
                 $dataCategories = mysqli_query($conn, $sqlCategories);
                 if (mysqli_num_rows($dataCategories)>0) { // if there're categories then go ahead >>
                         ...
                         echo "
                             <script>
                                 function category$rowCategories[id](){
                                     ";
                                     $sqlCategories1 = "SELECT * FROM categories WHERE id !=$rowCategories[id]";
